...
// Forwarder_udp represents a UDP packet forwarder_udp.
type Forwarder_udp struct {
	src          *net.UDPAddr
	dst          *net.UDPAddr
	client       *net.UDPAddr
...
func Forward_udp(src, dst string, timeout time.Duration) (*Forwarder_udp, error) {
	forwarder_udp := new(Forwarder_udp)
	forwarder_udp.connectCallback = func(addr string) {}
	forwarder_udp.disconnectCallback = func(addr string) {}
	...
	forwarder_udp.dst, err = net.ResolveUDPAddr("udp", dst)
	if err != nil {
		return nil, err
	}

	forwarder_udp.listenerConn, err = net.ListenUDP("udp", forwarder_udp.src)
	if err != nil {
		return nil, err
	}

	go forwarder_udp.janitor()
	go forwarder_udp.run()

	return forwarder_udp, nil
}

func (f *Forwarder_udp) run() {
	for {
...
func (f *Forwarder_udp) handle(data []byte, addr *net.UDPAddr) {
	f.connectionsMutex.Lock()
	...
	log.Println("sent packet to server", conn.udp.RemoteAddr()) // TODO this was commented out
...
// Close stops the forwarder_udp.
func (f *Forwarder_udp) Close() {
	f.connectionsMutex.Lock()
	f.closed = true
...
// new client connects.
func (f *Forwarder_udp) OnConnect(callback func(addr string)) {
...
func (f *Forwarder_udp) OnDisconnect(callback func(addr string)) {
	f.disconnectCallback = callback
}

// Connected returns the list of connected clients in IP:port form.
func (f *Forwarder_udp) Connected() []string {
	f.connectionsMutex.Lock()
